...
const tmpl8 = (template, attributes) => {
  try {
    ...
    templateStr += `const {${attributes.filter(a => !(/[\W_]+/g).test(a)).join(',')}} = data;let str='`;
    
    templateStr += template
      .replace(/\{\{\?\s*([\s\S]*?)\s*\}\}/g, (_match, arg) => arg ? `';if (${arg}){str+='` : `';}str+='`)
      .replace(/\{\{([\s\S]+?)\}\}/g, (_match, arg) => {
        const parts = arg.match(/^[ ]*([~!@#])[ ]*([\S\s]*)/);
        if (!parts) return `';str+=data['${arg}'];str+='`;
        const [ __match, type, rest ] = parts;
        const [ attr, format ] = rest.split(/\s*\|\s*/);
        if (type === '~' && format) return `';str+=B(data['${attr}'],'${format}');str+='`;
        if (type === '~') return `';str+=B(data['${attr}']);str+='`;
        if (type === '!') return `';str+=Boolean(data['${attr}']).toString();str+='`;
        if (type === '@' && format) return `';str+=D(data['${attr}'],'${format}');str+='`;
        if (type === '@') return `';str+=D(data['${attr}']);str+='`;
        if (type === '#' && format) return `';str+=N(data['${attr}'],'${format}');str+='`;
    ...
    return new Function("data", templateStr);
  } catch (e) {
